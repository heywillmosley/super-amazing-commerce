	 ...
	 */
	 	function theSiteName()
		{
			if( !defined( 'SITE_NAME' ) ) 
				define( 'SITE_NAME', FW_NAME );
				
			return SITE_NAME;
				
		} // end function theSiteName()
					...
					else
					{
						...
						if ( $this->STH->rowCount() >= 15 )
						{
							...
							if ( $this->STH->rowCount() >= 10 || $this->STH->rowCount() == 20 )
								$message = "Attempted brute force of username/email: " . $this->uoe['output'] . " from IP: " . $_SERVER['REMOTE_ADDR'] . " on ". the_site_name() . " at ". SITE_URL . '. ' . $this->STH->rowCount() . " Attempts. Time UTC: " . date( 'Y-m-d H:i:s', time() . 'TAKE IMMEDIATE ACTION!' );
				...
				{ 
					...
					?>
						<?php $this->form->openForm(); ?>
							...
							<h3 class="pull-right"><small class="text-muted txtR"><?php echo the_site_name(); ?></small></h3>
							<hr class="mbn"/>
					...
					{ ?>
						...
						<?php $this->form->openForm( 'form-inline mxw600 pan', '', FALSE, 'form' ); ?>
							...
							<h3 class="pull-right sr-only"><small class="text-muted txtR"><?php echo the_site_name(); ?></small></h3>
							<?php if( isset( $this->login_errors['mm_credentials'] ) ) 
								echo $this->login_errors['mm_credentials']; ?>
